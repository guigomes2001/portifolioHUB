...
public class UsuarioDAO extends DAOGenerica {
	...
	final static String SQL_CADASTRAR_USUARIO =  new StringBuilder()
			.append("INSERT INTO API_USUARIOS(LOGIN, SENHA, DATA_CADASTRO) ")
	...
	final static String SQL_CAPTURAR_USUARIO_POR_CHAVE = new StringBuilder()
			.append(" SELECT * FROM API_USUARIOS ")
			.append("   WHARE DATA_EXCLUSAO IS NULL ").toString();
	
	final static String SQL_ATUALIZAR_USUARIO = new StringBuilder()
			.append(" UPDATE API_USUARIOS ")
			.append("   SET TIPO_USUARIO = ? ")
	...
	public UsuarioDTO capturarUsuarioPorChave(Long chave) {
		...
		try {
			pstmt = getConexao().prepareStatement(SQL_CAPTURAR_USUARIO_POR_CHAVE);
	...
	public int atualizarUsuario(UsuarioDTO usuario) {
		...
		int i = 0;
		try {
			pstmt = getConexao().prepareStatement(SQL_ATUALIZAR_USUARIO);
